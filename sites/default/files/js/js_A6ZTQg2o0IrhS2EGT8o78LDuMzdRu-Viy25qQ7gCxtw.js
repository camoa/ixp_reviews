...
(function($,Drupal){Drupal.behaviors.ecaTableFilterByText={attach(context,settings){const [input]=once('models-filter-text','input.models-filter-text');if(!input)return;const $table=$(input.getAttribute('data-table'));let $rows;function filterModelList(e){const query=e.target.value.toLowerCase();function showModelRow(index,row){const sources=row.querySelectorAll('[data-drupal-selector="models-table-filter-text-source"]');let sourcesConcat='';sources.forEach((item)=>{sourcesConcat+=item.textContent;});const textMatch=sourcesConcat.toLowerCase().indexOf(query)!==-1;$(row).closest('tr').toggle(textMatch);}if(query.length>=2)$rows.each(showModelRow);else $rows.show();}if($table.length){$rows=$table.find('tbody tr');$(input).on('keyup',filterModelList);}}};})(jQuery,Drupal);;
